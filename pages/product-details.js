...
function ProductPage() {
  ...
  useEffect(() => {
    ...
    if (queryParam.liveAuction != undefined) {
      ...
      console.log("explor", JSON.parse(queryParam.explore).image_url);
    }
  }, [queryParam]);
  // useEffect(() => {
  //   ta && loadAsset(ta, ti);
        ...
        {asset.asset?.collection ? (
          ...
          <Content className={`d-sm-flex`}>
            ...
            <ItemInfo className={"float-none float-sm-left"}>
              <ItemDetails>
                <ItemDetailsHeader>
                  <ItemName>{asset.collection?.name}</ItemName>
                  <ItemTopButtonContainer>
                    <button
                      style={{
                        flex: "none",
                  ...
                  <BidOwnerContainer className={"border-right pr-2 pl-2"}>
                    ...
                    <BidPrice>
                      ...
                      <BidPriceValue>
                        <PriceInCryptoContainer>
                          <span>{`0.02 eth`}</span>
                          {/* <span>{`${getAuctionPriceDetails(asset.orders[0]).priceBase} eth`}</span> */}
                        </PriceInCryptoContainer>
                        <PriceInDollarContainer>
                          <span>{"~$32"}</span>
                        </PriceInDollarContainer>
