...
import {Button, CardTitle, ProductPrice, ProductCard, HeartIcon, NumberOfLikes, LikesContainer, ProductList, ProductCardHeader, ProductDescription} from "./StyledComponents/liveActions-styledComponents";
        ...
        <>
            <div >
                <Row>
                    ...
                    {PRODUCTS.map(n =>
                        <Col 
                        style={{width: "280px"}} className="p-3 p-sm-2 p-md-2 p-lg-3">
                            <ProductCard className={`p-2 p-lg-1`}>
                                ...
                                <Avatar.Group>
                                    {PRODUCTS.map(m =>
                                            ...
                                            <Avatar icon={<img src={m.productImage} width={12} height={12} />} />
                                            </Tooltip>
                                    )}
                                    </Avatar.Group>
                                </div>
                                ...
                                <ProductDescription>
                                <Link href={`/product-details?id=${n.id}`}><CardTitle>{n.productTitle}</CardTitle></Link>
                                <ProductPrice>{n.price}</ProductPrice>
                                <ProductList>{" "+n.currentQTY + ' of ' + n.totalQTY}</ProductList> <br/>
